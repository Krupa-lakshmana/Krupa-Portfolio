...
// ── Ticker data ────────────────────────────────────────────────────────────────
const TICKER_ITEMS = [
  "Python", "Java", "Spring Boot", "FastAPI", "Docker",
  "Kafka", "AWS", "PostgreSQL", "MongoDB", "REST APIs",
  "Microservices", "CI/CD", "Terraform", "LangChain", "Redis",
];
...
export const HeroSection = () => {
  ...
  return (
    <>
      {/* ── SKILLS TICKER ── */}
      <div className="w-full overflow-hidden border-b border-primary/10 bg-background/40 backdrop-blur-sm py-2.5 relative z-40">
        <div
          className="flex gap-10 whitespace-nowrap"
          style={{ animation: "ticker 30s linear infinite", width: "max-content" }}
        >
          {[...TICKER_ITEMS, ...TICKER_ITEMS].map((item, i) => (
            <span key={i} className="text-[11px] font-mono tracking-widest uppercase flex items-center gap-2 text-primary/70">
